...
# Word finder based on frequency and rules
def find_word_with_most_high_freq_letters(words, required_letters=[], forbidden_letters=[], known_letters="", previous_guesses=[]):
    ...
    for letter, _ in frequencies:
        ...
        more_filtered_words = [word for word in filtered_words if all(l in word for l in used_letters)]
        if more_filtered_words:
            return more_filtered_words
...
counter = 0
bug_fixing = False
possible_words = words_l  # Replace with ["allie"] to debug
