# Wordle AI with Correct History Handling and Full Pipeline
import random
import torch
...
# Prepare word list (lowercase, length 5)
word_list = [w.lower() for w in words.words() if len(w) == WORD_LENGTH and w.isalpha()]
word_list = list(set(word_list))  # Unique words
...
def encode_history(history):
    ...
    # Pad up to MAX_GUESSES
    padding_needed = MAX_GUESSES - len(history)
    enc.extend([0] * padding_needed * WORD_LENGTH * (26 + 3))
    return enc
...
# Simulate guessing a single secret word using the model
...
print("\n📊 Running full evaluation and saving results...")
results = {}
with open("wordle_results.csv", "w", newline="") as f:
    ...
    for i, secret in enumerate(word_list):
        ...
        if (i + 1) % 500 == 0:
            print(f"Progress: {i+1}/{len(word_list)}")
